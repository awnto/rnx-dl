...
#curl "https://raw.awnto.com/rnx/inet3server.php?rdu=$rdu&rfl=$rnx_rfl&ver=`rnx version`&date=`date +%Y%m%d`_`date +%H%M%S`&ip6=$(curl -6 ifconfig.me)"
curl "https://remote2.awnto.com/rnx/inet3server.php?rdu=$rdu&rfl=$rnx_rfl&ver=`rnx version`&aarch=`uname -m`&date=`date +%Y%m%d`_`date +%H%M%S`&ip6=$(curl -6 ifconfig.me)"
